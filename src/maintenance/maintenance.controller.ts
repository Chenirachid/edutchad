...
const CLE_MAINTENANCE = 'cheni-seed-2026';
...
export class MaintenanceController {
  ...
  @Get('seed-etablissement-exemple')
  async seedEtablissementExemple(@Query('cle') cle: string) {
    ...
    const code = await (async () => {
      ...
      let c = base;
      let i = 1;
      while (await this.prisma.etablissement.findUnique({ where: { code: c } })) {
        i++;
        c = `${base}-${i}`;
      }
      return c;
    ...
    const profsDef: [string, string, string[]][] = [
      ['Nadia', 'Boukar', ['Mathématiques']],
      ['Ismael', 'Youssouf', ['Français']],
      ['Fatimé', 'Abakar', ['Histoire-Géographie']],
      ['Moussa', 'Idriss', ['Anglais', 'SVT']],
    ];
    ...
    // Enseignements : classe x matière x prof
    ...
    return {
      message: 'Établissement exemple créé avec succès',
